...
class StandardStream : public std::streambuf
{
private:
    ...
    int originalFd = -1; // Original stdout file descriptor
    int pipeFd[2] = {-1, -1}; // Pipe for capturing stdout
    bool fdRedirected = false; // Track if file descriptor is redirected
    
    StandardStream() = default; // Private constructor for singleton pattern

    // Disable copy constructor and assignment operator
    StandardStream(const StandardStream&) = delete;
    StandardStream& operator=(const StandardStream&) = delete;
...
public:
    ...
    // Redirect stdout to this stream buffer (both C++ streams and file descriptor)
    bool redirect() {
        std::lock_guard<std::mutex> lock(mutex_);
        ...
        //     // Redirect file descriptor for printf
        //     if (!fdRedirected) {
        //         // Create a pipe
        //         if (pipe(pipeFd) == 0) {
        //             // Save original stdout
        //             originalFd = dup(STDOUT_FILENO);
        ...
        //                             // Also write to original stdout if available
        //                             if (originalFd != -1) {
    ...
    // Restore original stdout (both C++ stream and file descriptor)
    void restore() {
        std::lock_guard<std::mutex> lock(mutex_);
    ...
    template<typename T>
    StandardStream& operator<<(const T& value) {
    ...
    StandardStream& operator<<(const std::string& str) {
        xsputn(str.c_str(), str.length());
        return *this;
    }

    // Specialized operator for C-strings
    StandardStream& operator<<(const char* str) {
        if (str) {
    ...
    StandardStream& operator<<(std::ostream& (*manipulator)(std::ostream&)) {
    ...
    static StandardStream& getInstance() {
        static StandardStream instance;
        return instance;
    }
