...
public:
    ...
    std::vector<std::string> getAllTagsPublic()
    {
        ...
        if (dbBackend && dbBackend->isOpen())
        {
            ...
            if (!stmt)
            {
                PLOGW << "getAllTags prepare failed: " << err;
            }
            else
            {
    ...
    void enqueueMainThreadTask(std::function<void()> fn)
    {
    ...
    struct ItemRecord
    {
        ...
        std::string content;
        std::string tags;
        int64_t versionSeq = 0;
        std::string headRevision;
        int isRoot = 0;
    };
    ItemRecord getItemPublic(int64_t id)
    {
        ItemRecord out;
        out.id = id;
        if (dbBackend && dbBackend->isOpen())
        {
            ...
            if (stmt)
            {
                ...
                if (rs && rs->next())
                {
                    out.name = rs->getString(0);
                    out.content = rs->getString(1);
                    out.tags = rs->getString(2);
                    out.versionSeq = rs->getInt64(3);
                    out.headRevision = rs->getString(4);
                    out.isRoot = rs->getInt(5);
                }
            }
        }
        else if (dbConnection)
        {
            ...
            const char *q = "SELECT Name, Content, Tags, VersionSeq, HeadRevision, IsRoot FROM VaultItems WHERE ID = ? LIMIT 1;";
            if (sqlite3_prepare_v2(dbConnection, q, -1, &s, nullptr) == SQLITE_OK)
            {
                ...
                if (sqlite3_step(s) == SQLITE_ROW)
                {
                    ...
                    if (n)
                        out.name = reinterpret_cast<const char *>(n);
                    if (c)
                        out.content = reinterpret_cast<const char *>(c);
                    if (t)
                        out.tags = reinterpret_cast<const char *>(t);
                    if (sqlite3_column_type(s, 3) != SQLITE_NULL)
                        out.versionSeq = sqlite3_column_int64(s, 3);
                    const unsigned char *hr = sqlite3_column_text(s, 4);
                    if (hr)
                        out.headRevision = reinterpret_cast<const char *>(hr);
                    out.isRoot = sqlite3_column_int(s, 5);
                }
            }
            if (s)
                sqlite3_finalize(s);
        ...
        try
        {
            ...
            if (loadedItemID >= 0)
            {
                if (dbBackend && dbBackend->isOpen())
                {
                    ...
                    if (!stmt)
                    {
                        PLOGW << "replace content prepare failed: " << err;
                    }
                    else
                    {
                        stmt->bindString(1, currentContent);
                        stmt->bindInt(2, loadedItemID);
                        if (!stmt->execute())
                            PLOGW << "replace content execute failed";
                    }
                    contentDirty = false;
    ...
    Vault(std::filesystem::path dbPath, std::string vaultName)
    {
        ...
        errMsg = nullptr;
        if (sqlite3_exec(dbConnection, createFiltersTableSQL, nullptr, nullptr, &errMsg) != SQLITE_OK)
        {
            if (errMsg)
                sqlite3_free(errMsg);
        }
        ...
        errMsg = nullptr;
        if (sqlite3_exec(dbConnection, createMetaSQL, nullptr, nullptr, &errMsg) != SQLITE_OK)
        {
            if (errMsg)
                sqlite3_free(errMsg);
        }
        ...
        const char *createAttachmentsSQL = "CREATE TABLE IF NOT EXISTS Attachments ("
                                           ...
                                           ");";
        errMsg = nullptr;
        if (sqlite3_exec(dbConnection, createAttachmentsSQL, nullptr, nullptr, &errMsg) != SQLITE_OK)
        {
            if (errMsg)
                sqlite3_free(errMsg);
        }
        ...
        // Migration: ensure DisplayWidth and DisplayHeight columns exist on older DBs
        {
            ...
            if (sqlite3_prepare_v2(dbConnection, pragmaSQL, -1, &pragma, nullptr) == SQLITE_OK)
            {
                while (sqlite3_step(pragma) == SQLITE_ROW)
                {
                    ...
                    if (colName)
                    {
                        ...
                        if (cname == "DisplayWidth")
                            hasDisplayWidth = true;
                        if (cname == "DisplayHeight")
                            hasDisplayHeight = true;
                    }
                }
            }
            if (pragma)
            ...
            if (!hasDisplayWidth)
            {
                ...
                {
                    PLOGI << "vault:migration added DisplayWidth column";
                }
            }
            if (!hasDisplayHeight)
            {
                ...
                {
                    PLOGI << "vault:migration added DisplayHeight column";
                }
            }
        }
        ...
        errMsg = nullptr;
        if (sqlite3_exec(dbConnection, idxSQL, nullptr, nullptr, &errMsg) != SQLITE_OK)
        {
            if (errMsg)
                sqlite3_free(errMsg);
        }
        ...
        // Migration: detect old Attachments schema where ItemID was declared NOT NULL, and rebuild table if necessary
        {
            ...
            if (sqlite3_prepare_v2(dbConnection, pragmaSQL, -1, &pragma, nullptr) == SQLITE_OK)
            {
                while (sqlite3_step(pragma) == SQLITE_ROW)
                {
                    ...
                    if (colName && std::string(reinterpret_cast<const char *>(colName)) == "ItemID" && notnull == 1)
                    {
                        needRebuild = true;
                        break;
                    }
            ...
            if (needRebuild)
            {
                ...
                char *mErr = nullptr;
                if (sqlite3_exec(dbConnection, beginT, nullptr, nullptr, &mErr) != SQLITE_OK)
                {
                    if (mErr)
                        sqlite3_free(mErr);
                }
                mErr = nullptr;
                if (sqlite3_exec(dbConnection, createNew, nullptr, nullptr, &mErr) != SQLITE_OK)
                {
                    if (mErr)
                        sqlite3_free(mErr);
                }
                mErr = nullptr;
                if (sqlite3_exec(dbConnection, copySQL, nullptr, nullptr, &mErr) != SQLITE_OK)
                {
                    if (mErr)
                        sqlite3_free(mErr);
                }
                mErr = nullptr;
                if (sqlite3_exec(dbConnection, dropOld, nullptr, nullptr, &mErr) != SQLITE_OK)
                {
                    if (mErr)
                        sqlite3_free(mErr);
                }
                mErr = nullptr;
                if (sqlite3_exec(dbConnection, rename, nullptr, nullptr, &mErr) != SQLITE_OK)
                {
                    if (mErr)
                        sqlite3_free(mErr);
                }
                mErr = nullptr;
                if (sqlite3_exec(dbConnection, idxSQL, nullptr, nullptr, &mErr) != SQLITE_OK)
                {
                    if (mErr)
                        sqlite3_free(mErr);
                }
                mErr = nullptr;
                if (sqlite3_exec(dbConnection, commitT, nullptr, nullptr, &mErr) != SQLITE_OK)
                {
                    if (mErr)
                        sqlite3_free(mErr);
                }
        ...
        const char *createUsersSQL = "CREATE TABLE IF NOT EXISTS Users ("
                                     ...
                                     ");";
        errMsg = nullptr;
        if (sqlite3_exec(dbConnection, createUsersSQL, nullptr, nullptr, &errMsg) != SQLITE_OK)
        {
            if (errMsg)
                sqlite3_free(errMsg);
        }
        ...
        const char *createPermsSQL = "CREATE TABLE IF NOT EXISTS ItemPermissions ("
                                     ...
                                     ");";
        errMsg = nullptr;
        if (sqlite3_exec(dbConnection, createPermsSQL, nullptr, nullptr, &errMsg) != SQLITE_OK)
        {
            if (errMsg)
                sqlite3_free(errMsg);
        }
        ...
        errMsg = nullptr;
        if (sqlite3_exec(dbConnection, idxPermsSQL, nullptr, nullptr, &errMsg) != SQLITE_OK)
        {
            if (errMsg)
                sqlite3_free(errMsg);
        }
        ...
        // Ensure SchemaVersion entry exists (set to 3 for user support)
        {
            ...
            if (sqlite3_prepare_v2(dbConnection, selectVer, -1, &verStmt, nullptr) == SQLITE_OK)
            {
                if (sqlite3_step(verStmt) == SQLITE_ROW)
                {
                    ...
                    if (v)
                        verInt = std::atoi(reinterpret_cast<const char *>(v));
                    hasVer = true;
                }
            }
            if (verStmt)
            ...
            if (!hasVer || verInt < 3)
            {
                ...
                char *iErr = nullptr;
                if (sqlite3_exec(dbConnection, insertVer, nullptr, nullptr, &iErr) != SQLITE_OK)
                {
                    if (iErr)
                        sqlite3_free(iErr);
                }
        ...
        // Ensure legacy DBs get the IsRoot column so the root can be identified by ID instead of Name
        {
            ...
            // Ensure per-item VersionSeq and HeadRevision columns exist for revision history
            {
                ...
                if (sqlite3_prepare_v2(dbConnection, pragmaSQL2, -1, &pragma2, nullptr) == SQLITE_OK)
                {
                    while (sqlite3_step(pragma2) == SQLITE_ROW)
                    {
                        ...
                        if (colName)
                        {
                            ...
                            if (cname == "VersionSeq")
                                hasVersionSeq = true;
                            if (cname == "HeadRevision")
                                hasHeadRevision = true;
                        }
                    }
                }
                if (pragma2)
                    sqlite3_finalize(pragma2);
                if (!hasVersionSeq)
                {
                ...
                if (!hasHeadRevision)
                {
                    ...
                    char *aErr = nullptr;
                    if (sqlite3_exec(dbConnection, alter2, nullptr, nullptr, &aErr) != SQLITE_OK)
                    {
                        if (aErr)
                            sqlite3_free(aErr);
                    }
    ...
    void loadNodeFiltersFromDB()
    {
        if (dbBackend && dbBackend->isOpen())
        {
            ...
            if (!stmt)
            {
                PLOGW << "loadNodeFilters prepare failed: " << err;
                return;
            }
            auto rs = stmt->executeQuery();
            while (rs && rs->next())
        ...
        if (sqlite3_prepare_v2(dbConnection, sql, -1, &stmt, nullptr) == SQLITE_OK)
        {
            while (sqlite3_step(stmt) == SQLITE_ROW)
            {
                ...
                ChildFilterSpec s;
                if (mode)
                    s.mode = reinterpret_cast<const char *>(mode);
                if (tags)
                {
                ...
                if (expr)
                    s.expr = reinterpret_cast<const char *>(expr);
                nodeChildFilters[nid] = s;
            }
        }
        if (stmt)
    ...
    void saveNodeFilterToDB(int64_t nodeID, const ChildFilterSpec &spec)
    {
        if (dbBackend && dbBackend->isOpen())
        {
            ...
            if (!stmt)
            {
                PLOGW << "saveNodeFilter prepare failed: " << err;
                return;
            }
    ...
    void clearNodeFilterFromDB(int64_t nodeID)
    {
        if (dbBackend && dbBackend->isOpen())
        {
            ...
            if (!stmt)
            {
                PLOGW << "clearNodeFilter prepare failed: " << err;
                return;
            }
            stmt->bindInt(1, nodeID);
            stmt->execute();
    ...
    int64_t getOrCreateRoot()
    {
        ...
        if (dbBackend && dbBackend->isOpen())
        {
            ...
            // Fall back to name-based lookup and set IsRoot
            {
                ...
                if (id != -1)
                {
                    auto u = dbBackend->prepare("UPDATE VaultItems SET IsRoot = 1 WHERE ID = ?;", &err);
                    if (u)
                    {
                        u->bindInt(1, id);
                        u->execute();
            ...
            // Find candidate with no parents
            {
                ...
                {
                    std::string err2;
                    auto u = dbBackend->prepare("UPDATE VaultItems SET IsRoot = 1 WHERE ID = ?;", &err2);
                    if (u)
                    {
                        u->bindInt(1, id);
                        u->execute();
            ...
            {
                std::string err;
                if (!dbBackend->execute("ALTER TABLE VaultItems ADD COLUMN IsRoot TINYINT DEFAULT 0;", &err))
                    PLOGW << "MySQL: failed to add IsRoot column: " << err;
            }
            // Insert new root
            {
                std::string err;
                ...
                if (!stmt)
                {
                    PLOGE << "getOrCreateRoot prepare failed: " << err;
                    return -1;
                }
                stmt->bindString(1, name);
                stmt->bindString(2, "");
                ...
                if (!stmt->execute())
                {
                    PLOGE << "getOrCreateRoot insert failed";
                    return -1;
                }
    ...
    void drawVaultTree()
    {
        ...
        if (ImGui::BeginPopupModal("Create Note", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
        {
            ...
            if (ImGui::Button("Create"))
            {
                ...
                if (selectedItemID >= 0)
                    inherited = collectTagsUnionFromNode(parent);
                ...
                if (nid != -1)
                {
                    if (!inherited.empty())
                    {
                        ...
                        std::string s;
                        for (size_t ii = 0; ii < inherited.size(); ++ii)
                        {
                            if (ii)
                                s += ",";
                            s += inherited[ii];
        ...
        ImGui::Text("Mode:");
        ImGui::SameLine();
        if (ImGui::RadioButton("All (AND)", tagFilterModeAll))
            tagFilterModeAll = true;
        ImGui::SameLine();
        if (ImGui::RadioButton("Any (OR)", !tagFilterModeAll))
            tagFilterModeAll = false;
        ...
        for (size_t i = 0; i < activeTagFilter.size(); ++i)
        {
            ...
            ImGui::TextUnformatted(activeTagFilter[i].c_str());
            ImGui::SameLine();
        ...
        if (!activeTagFilter.empty())
        {
                ...
                if (nodeMatchesActiveFilter(id))
                {
                    ...
                    if (ImGui::BeginPopupContextItem((std::string("ctx") + std::to_string(id)).c_str()))
                    {
                        ...
                        if (ImGui::MenuItem("Import Markdown..."))
                        {
                            ...
                            importParentID = id;
                            importParentPath.clear();
                            importParentPath.push_back(id);
            ...
            if (showImportModal)
            {
                ImGui::OpenPopup("Import Markdown Files");
            ...
            if (ImGui::BeginPopupModal("Import Markdown Files", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
            {
                ...
                ImGui::Separator();
                ImGui::TextWrapped("%s", importPath.string().c_str());
                ImGui::SameLine();
                if (ImGui::Button("Up"))
                {
                    if (importPath.has_parent_path())
                        importPath = importPath.parent_path();
                }
                ImGui::Separator();
                try
                {
                    ...
                    for (auto &f : files)
                    {
                        ...
                        {
                            if (sel)
                                importSelectedFiles.erase(fname);
                            else
                                importSelectedFiles.insert(fname);
                        }
                    }
                }
                catch (...)
                {
                    ImGui::TextColored(ImVec4(1, 0.4f, 0.4f, 1.0f), "Failed to read directory");
                }
                    ...
                    try
                    {
                        for (auto &e : std::filesystem::directory_iterator(importPath))
                        {
                            if (e.is_regular_file())
                            {
                                ...
                                std::transform(ext.begin(), ext.end(), ext.begin(), ::tolower);
                                if (ext == ".md" || ext == ".markdown")
                                    importSelectedFiles.insert(e.path().filename().string());
                            }
                        }
                    }
                    catch (...)
                ...
                if (ImGui::Button("Import"))
                {
                    ...
                    if (imported > 0)
                    {
                        statusMessage = std::string("Imported ") + std::to_string(imported) + " files";
                        statusTime = ImGui::GetTime();
                        if (lastId != -1)
        ...
        if (showSetFilterModal)
        {
            ImGui::OpenPopup("Set Child Filter");
            showSetFilterModal = false;
            setFilterSelectedTags = setFilterInitialTags;
            setFilterMode = setFilterModeDefault;
        }
        if (ImGui::BeginPopupModal("Set Child Filter", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
        {
            ...
            ImGui::Text("Mode:");
            ImGui::SameLine();
            ImGui::RadioButton("AND", &setFilterMode, 0);
            ImGui::SameLine();
            ImGui::RadioButton("OR", &setFilterMode, 1);
            ImGui::SameLine();
            ...
            if (setFilterMode != 2)
            {
                ...
                for (auto &t : allTags)
                {
                    ...
                    {
                        if (sel)
                            setFilterSelectedTags.erase(std::remove(setFilterSelectedTags.begin(), setFilterSelectedTags.end(), t), setFilterSelectedTags.end());
                        else
                            setFilterSelectedTags.push_back(t);
                    }
                }
                ImGui::Separator();
                // Add custom tag
                static char newTagBuf[64];
                ImGui::InputTextWithHint("##newtag", "Type a tag and press Add", newTagBuf, sizeof(newTagBuf));
                ImGui::SameLine();
                if (ImGui::Button("Add"))
                {
                    ...
                    if (!nt.empty())
                    {
                        if (std::find(setFilterSelectedTags.begin(), setFilterSelectedTags.end(), nt) == setFilterSelectedTags.end())
                            setFilterSelectedTags.push_back(nt);
                        newTagBuf[0] = '\0';
                    }
                }
                // show selected
                ImGui::Text("Selected:");
                for (size_t i = 0; i < setFilterSelectedTags.size(); ++i)
                {
                    ImGui::SameLine();
                    ImGui::TextDisabled("%s", setFilterSelectedTags[i].c_str());
                }
            }
            else
            {
            ...
            if (ImGui::Button("Set"))
            {
                ...
                {
                    spec.mode = "EXPR";
                    spec.expr = std::string(setFilterBuf);
                    spec.tags.clear();
                }
                else
                {
                    spec.mode = (setFilterMode == 1) ? "OR" : "AND";
                    spec.tags = setFilterSelectedTags;
                    spec.expr.clear();
                }
    ...
    bool nodeOrDescendantMatchesWithSpecs(int64_t nodeID, const std::vector<ChildFilterSpec> &inheritedSpecs, std::unordered_set<int64_t> &visited)
    {
        ...
        if (!inheritedSpecs.empty())
        {
            ...
            for (const auto &spec : inheritedSpecs)
            {
                if (!evalFilterSpecOnTags(spec, tagsLower))
                {
                    ok = false;
                    break;
                }
        ...
        if (it != nodeChildFilters.end())
            nextSpecs.push_back(it->second);
        std::vector<int64_t> children;
        getChildren(nodeID, children);
        for (auto c : children)
        {
            if (nodeOrDescendantMatchesWithSpecs(c, nextSpecs, visited))
                return true;
        }
        return false;
    }
    ...
    bool nodeOrDescendantVisible(int64_t nodeID, int64_t userID, std::unordered_set<int64_t> &visited)
    {
        if (visited.find(nodeID) != visited.end())
            return false; // cycle protection
        visited.insert(nodeID);
        if (isItemVisibleToUser(nodeID, userID))
            return true;
        std::vector<int64_t> children;
        getChildren(nodeID, children);
        for (auto c : children)
        {
            if (nodeOrDescendantVisible(c, userID, visited))
                return true;
        }
        return false;
    }
    ...
    static bool evalFilterExpr(const std::string &expr, const std::unordered_set<std::string> &tagsLower)
    {
        ...
        struct P
        {
            ...
            P(const std::string &s_, const std::unordered_set<std::string> &t_) : s(s_), p(0), tags(t_) {}
            void skip()
            {
            ...
            bool parsePrimary()
            {
                ...
                skip();
                size_t start = p;
                while (p < s.size() && (isalnum((unsigned char)s[p]) || s[p] == '_' || s[p] == '-' || s[p] == '.'))
                    ++p;
                if (p > start)
                {
    ...
    static bool evalFilterSpecOnTags(const ChildFilterSpec &spec, const std::unordered_set<std::string> &tagsLower)
    {
        ...
        else if (spec.mode == "OR")
        {
            ...
            for (auto &t : spec.tags)
            {
                ...
                if (tagsLower.find(low) != tagsLower.end())
                    return true;
            }
            return false;
        }
    ...
    void drawVaultContent()
    {
        ...
        if (loadedItemID != selectedItemID)
        {
            if (contentDirty && loadedItemID >= 0)
            {
                ...
                if (dbBackend && dbBackend->isOpen())
                {
                    ...
                    if (!stmt)
                    {
                        PLOGW << "save content prepare failed: " << err;
                    }
                    else
                    {
                        stmt->bindString(1, currentContent);
                        stmt->bindInt(2, loadedItemID);
                        if (!stmt->execute())
                            PLOGW << "save content execute failed";
                    }
                    contentDirty = false;
                    lastSaveTime = ImGui::GetTime();
                }
            ...
            if (dbBackend && dbBackend->isOpen())
            {
                ...
                if (!stmt)
                {
                    PLOGW << "load content prepare failed: " << err;
                }
                else
                {
            ...
            else if (dbConnection)
            {
                ...
                if (sqlite3_prepare_v2(dbConnection, sql, -1, &stmt, nullptr) == SQLITE_OK)
                {
                    ...
                    if (sqlite3_step(stmt) == SQLITE_ROW)
                    {
                        ...
                        if (nameText)
                            currentTitle = reinterpret_cast<const char *>(nameText);
                        if (contentText)
                            currentContent = reinterpret_cast<const char *>(contentText);
                        if (tagsText)
                            currentTags = reinterpret_cast<const char *>(tagsText);
                    }
                }
                if (stmt)
                    sqlite3_finalize(stmt);
        ...
        if (maxBottomMeta < minBottomMeta)
            maxBottomMeta = minBottomMeta;
        if (bottomMetaHeight < minBottomMeta)
            bottomMetaHeight = minBottomMeta;
        if (bottomMetaHeight > maxBottomMeta)
            bottomMetaHeight = maxBottomMeta;
            ...
            if (ImGui::InputText("Title", titleBuf, sizeof(titleBuf)))
            {
                ...
                if (loadedItemID >= 0)
                {
                    if (dbBackend && dbBackend->isOpen())
                    {
                        ...
                        if (!stmt)
                        {
                            PLOGW << "update title prepare failed: " << err;
                        }
                        else
                        {
                            stmt->bindString(1, newTitle);
                            stmt->bindInt(2, loadedItemID);
                            if (!stmt->execute())
                                PLOGW << "update title execute failed";
                        }
            ...
            if (ImGui::InputTextMultiline("##md_editor", &currentContent, editorSize, ImGuiInputTextFlags_AllowTabInput))
            {
                ...
                if (loadedItemID >= 0)
                {
                    if (dbBackend && dbBackend->isOpen())
                    {
                        ...
                        if (!stmt)
                        {
                            PLOGW << "update content prepare failed: " << err;
                        }
                        else
                        {
                            stmt->bindString(1, currentContent);
                            stmt->bindInt(2, loadedItemID);
                            if (!stmt->execute())
                                PLOGW << "update content execute failed";
                        }
                        contentDirty = false;
                        lastSaveTime = ImGui::GetTime();
                    }
        ...
        // Process pending model viewer reloads triggered by background fetches
        {
            std::vector<std::string> pending;
            {
                ...
                if (!pendingViewerReloads.empty())
                    pending.swap(pendingViewerReloads);
            }
            for (const auto &k : pending)
            {
                ...
                if (it != modelViewerCache.end())
                {
                    ...
                    if (k.rfind(vpfx, 0) == 0)
                    {
                        try
                        {
                            aid = std::stoll(k.substr(vpfx.size()));
                        }
                        catch (...)
                        {
                        }
                        ...
                        aid = findAttachmentByExternalPath(k);
                    if (aid != -1)
                    {
        ...
        // Execute any queued main-thread tasks (e.g., GL uploads) that were enqueued by background workers
        {
            ...
            {
                std::lock_guard<std::mutex> l(pendingTasksMutex);
                if (!pendingMainThreadTasks.empty())
                    tasks.swap(pendingMainThreadTasks);
            }
            for (auto &t : tasks)
                t();
        }
        ...
        for (size_t i = 0; i < tags.size(); ++i)
        {
            ...
            {
                ImGui::BeginDisabled();
                ImGui::SmallButton((std::string("x##tag") + std::to_string(i)).c_str());
                ImGui::EndDisabled();
            }
            ImGui::SameLine();
            ImGui::PopID();
        ...
        for (auto pid : parents)
        {
            ...
            ImGui::SameLine();
            ImGui::TextUnformatted(getItemName(pid).c_str());
            ImGui::SameLine();
        ...
        else
        {
            ...
            if (remaining < ImGui::GetTextLineHeightWithSpacing())
                remaining = ImGui::GetTextLineHeightWithSpacing();
            ...
            for (auto &it : all)
            {
                ...
                std::string filter = parentFilter;
                std::transform(lowerName.begin(), lowerName.end(), lowerName.begin(), ::tolower);
                std::transform(filter.begin(), filter.end(), filter.begin(), ::tolower);
                if (filter.size() && lowerName.find(filter) == std::string::npos)
                    continue;
                ...
                {
                    ImGui::BeginDisabled();
                    ImGui::Selectable(selLabel.c_str());
                    ImGui::EndDisabled();
                }
            }
            ImGui::EndChild();
        ...
        ImGui::SameLine();
        {
            ...
            if (currentUserID > 0)
            {
                ...
                if (sqlite3_prepare_v2(dbConnection, aSql, -1, &astmt, nullptr) == SQLITE_OK)
                {
                    ...
                    if (sqlite3_step(astmt) == SQLITE_ROW)
                        currentIsAdmin = (sqlite3_column_int(astmt, 0) != 0);
                }
                if (astmt)
            ...
            if (users.empty())
                ImGui::TextDisabled("No users defined");
            for (auto &u : users)
            {
                ...
                {
                    ImGui::BeginDisabled();
                    ImGui::Combo((std::string("##perm") + std::to_string(u.id)).c_str(), &sel, opts, 3);
                    ImGui::EndDisabled();
                }
                ImGui::SameLine();
        ...
        if (canEdit)
        {
            ...
            ImGui::SameLine();
            if (ImGui::Button("Import Attachment"))
            {
                std::string p(attachPathBuf);
                if (!p.empty())
                {
        ...
        else
        {
            ...
            ImGui::SameLine();
            ImGui::Button("Import Attachment");
            ImGui::SameLine();
        ...
        for (auto &a : attachments)
        {
            ...
            ImGui::TextUnformatted(label.c_str());
            ImGui::SameLine();
            ImGui::TextDisabled("(%lld bytes)", (long long)a.size);
            ImGui::SameLine();
            if (ImGui::Button((std::string("Preview##att") + std::to_string(a.id)).c_str()))
            {
                previewAttachmentID = a.id;
                previewIsRaw = false;
                auto meta = getAttachmentMeta(a.id);
                previewDisplayWidth = meta.displayWidth;
                previewDisplayHeight = meta.displayHeight;
                showAttachmentPreview = true;
                ImGui::OpenPopup("Attachment Preview");
            }
            ImGui::SameLine();
            if (ImGui::SmallButton((std::string("Remove##att") + std::to_string(a.id)).c_str()))
            {
                if (removeAttachment(a.id))
                {
        ...
        if (showAttachmentPreview)
        {
            if (ImGui::BeginPopupModal("Attachment Preview", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
            {
                if (previewIsRaw && previewAttachmentID == -1)
                {
                    ...
                    ImGui::Separator();
                    // Save As
                    ImGui::InputText("##savepath", attachmentSavePathBuf, sizeof(attachmentSavePathBuf));
                    ImGui::SameLine();
                    if (ImGui::Button("Save As"))
                    {
                        ...
                        if (!outp.empty())
                        {
                            ...
                            if (of)
                            {
                                of.write(reinterpret_cast<const char *>(previewRawData.data()), previewRawData.size());
                                of.close();
                                statusMessage = "Saved";
                                statusTime = ImGui::GetTime();
                            }
                    ...
                    if (ImGui::Button("Reset"))
                    {
                        ...
                        statusMessage = "Display reset (temporary)";
                        statusTime = ImGui::GetTime();
                    }
                }
                else
                {
                    ...
                    if (ImGui::Button("Save Display"))
                    {
                        ...
                        {
                            statusMessage = "Display saved";
                            statusTime = ImGui::GetTime();
                        }
                        else
                        {
                            statusMessage = "Failed to save display";
                            statusTime = ImGui::GetTime();
                        }
                    }
                    ImGui::SameLine();
                    if (ImGui::Button("Reset"))
                    {
                        previewDisplayWidth = 0;
                        previewDisplayHeight = 0;
                        setAttachmentDisplaySize(meta.id, -1, -1);
                ...
                auto checkAndShowModelBtn = [&](const std::string &mime, const std::string &filename, const std::vector<uint8_t> &raw, int64_t attID) -> void
                {
                    auto isModelExt = [&](const std::string &f) -> bool
                    {
                        ...
                        for (auto &m : models)
                            if (ext == m)
                                return true;
                        if (mime.find("model") != std::string::npos)
                            return true;
                        return false;
                    };
                    if (isModelExt(filename))
                    {
                        ...
                        if (ImGui::Button("View Model"))
                        {
                            ...
                            else
                            {
                                ...
                                if (attID >= 0)
                                {
                                    auto meta = getAttachmentMeta(attID);
                                    if (!meta.externalPath.empty())
                                    {
                                        asyncFetchAndStoreAttachment(attID, meta.externalPath);
                                        statusMessage = "Fetching model...";
                                        statusTime = ImGui::GetTime();
                                    }
                                }
                                else
                                {
                                    statusMessage = "No data to load";
                                    statusTime = ImGui::GetTime();
                                }
                            }
                        }
        ...
        {
            ImGui::OpenPopup("Import Asset(s)");
        ...
        if (ImGui::BeginPopupModal("Import Asset(s)", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
        {
            ...
            ImGui::Separator();
            ImGui::TextWrapped("%s", importAssetPath.string().c_str());
            ImGui::SameLine();
            if (ImGui::Button("Up"))
            {
                if (importAssetPath.has_parent_path())
                    importAssetPath = importAssetPath.parent_path();
            }
            ImGui::Separator();
            try
            {
                ...
                for (auto &f : files)
                {
                    ...
                    {
                        if (sel)
                            importAssetSelectedFiles.erase(fname);
                        else
                            importAssetSelectedFiles.insert(fname);
                    }
                }
            }
            catch (...)
            ...
            if (ImGui::Button("Select All"))
            {
                try
                {
                    ...
                    {
                        if (e.is_regular_file())
                            importAssetSelectedFiles.insert(e.path().filename().string());
                    }
                }
                catch (...)
                {
            ...
            if (ImGui::Button("Upload"))
            {
                ...
                for (auto &fname : importAssetSelectedFiles)
                {
                    try
                    {
                        ...
                        // normalize
                        while (!dest.empty() && (dest.front() == '/' || dest.front() == '\\'))
                        ...
                        if (id > 0)
                        {
                            auto meta = getAttachmentMeta(id);
                            if (!meta.externalPath.empty())
        ...
        if (showAssetUploadedModal)
        {
            ImGui::OpenPopup("Asset Uploaded");
            showAssetUploadedModal = false;
        }
        ...
        if (showOverwriteConfirmModal)
        {
            ImGui::OpenPopup("Asset conflict");
            showOverwriteConfirmModal = false;
        }
        if (ImGui::BeginPopupModal("Asset conflict", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
        {
            ...
            if (ImGui::Button("Replace"))
            {
                ...
                lastUploadedExternalPaths.clear();
                lastUploadedExternalPaths.push_back(overwriteTargetExternalPath);
                showAssetUploadedModal = true;
                ImGui::CloseCurrentPopup();
            }
            ImGui::SameLine();
            if (ImGui::Button("Upload as Copy"))
            {
                ...
                if (!ext.empty())
                {
                    ...
                    if (lowext == ".png" || lowext == ".jpg" || lowext == ".jpeg" || lowext == ".bmp" || lowext == ".gif")
                        mime = std::string("image/") + (lowext.size() > 1 ? lowext.substr(1) : "");
                }
                ...
                {
                    lastUploadedExternalPaths.clear();
                    lastUploadedExternalPaths.push_back(std::string("vault://Assets/") + candidateRel);
                    showAssetUploadedModal = true;
                }
                ImGui::CloseCurrentPopup();
            }
        ...
        for (auto &kv : modelViewerCache)
            if (kv.second)
                kv.second->processPendingUploads();
        if (modelViewer)
            modelViewer->processPendingUploads();

        // Model viewer window (dockable)
        if (showModelViewer && modelViewer)
        {
            modelViewer->renderWindow("Model Viewer", &showModelViewer);
        }
    }
    ...
    int64_t createItem(const std::string &name, int64_t parentID = -1)
    {
        ...
        if (dbBackend && dbBackend->isOpen())
        {
            ...
            if (!stmt)
            {
                PLOGE << "createItem prepare failed: " << err;
                return -1;
            }
            stmt->bindString(1, name);
            stmt->bindString(2, "");
            ...
            if (!stmt->execute())
            {
                PLOGE << "createItem execute failed";
                return -1;
            }
        ...
        if (history)
        {
            ...
            if (rid.empty())
                PLOGW << "recordRevision failed for create item " << id;
        }
        return id;
    }
    ...
    int64_t createItemWithContent(const std::string &name, const std::string &content, const std::vector<std::string> &tags, int64_t parentID = -1)
    {
        ...
        if (dbBackend && dbBackend->isOpen())
        {
            ...
            if (!stmt)
            {
                PLOGE << "createItemWithContent prepare failed: " << err;
                return id;
            }
            ...
            stmt->bindInt(3, id);
            if (!stmt->execute())
                PLOGE << "createItemWithContent execute failed";
        ...
        if (history)
        {
            ...
            if (rid.empty())
                PLOGW << "recordRevision failed for createItemWithContent id=" << id;
        }
        return id;
    }
...
private:
    ...
    void drawVaultNode(int64_t parentID, int64_t nodeID, std::vector<int64_t> &path)
    {
        ...
        if (!activeTagFilter.empty())
        {
            ...
            if (!nodeOrDescendantMatches(nodeID, visited))
                return;
        }
        ...
        if (currentUserID != -1)
        {
            ...
            if (!nodeOrDescendantVisible(nodeID, currentUserID, vvisited))
                return;
        }
        ...
        if (fit != nodeChildFilters.end())
        {
            ...
            {
                if (!fit->second.expr.empty())
                    displayName += std::string(" [filter: EXPR: ") + fit->second.expr + "]";
            }
            else if (!fit->second.tags.empty())
            {
                std::string fstr;
                for (size_t i = 0; i < fit->second.tags.size(); ++i)
                {
                    if (i)
                        fstr += ",";
                    fstr += fit->second.tags[i];
                }
        ...
        if (children.empty())
        {
            ...
            if (ImGui::BeginPopupContextItem("node_context"))
            {
                ...
                if (ImGui::MenuItem("Import Markdown..."))
                {
                    ...
                    importParentPath.push_back(nodeID);
                    importPath = std::filesystem::current_path();
                    importSelectedFiles.clear();
                }
                if (ImGui::MenuItem("Set Child Filter..."))
                {
                    ...
                    if (it != nodeChildFilters.end())
                    {
                        if (it->second.mode == "OR")
                            setFilterModeDefault = 1;
                        else if (it->second.mode == "EXPR")
                            setFilterModeDefault = 2;
                        else
                            setFilterModeDefault = 0;
                        setFilterInitialTags = it->second.tags;
                        if (!it->second.expr.empty())
                            strncpy(setFilterBuf, it->second.expr.c_str(), sizeof(setFilterBuf));
                    }
        ...
        else
        {
            ...
            if (open)
            {
                ...
                for (auto anc : path)
                {
                    ...
                    if (it != nodeChildFilters.end())
                        inheritedSpecs.push_back(it->second);
                }
                // Also include this node's filter for its children
                auto itcur = nodeChildFilters.find(nodeID);
                if (itcur != nodeChildFilters.end())
                    inheritedSpecs.push_back(itcur->second);
                ...
                for (auto child : children)
                {
                    ...
                    if (!inheritedSpecs.empty())
                    {
                        if (!nodeOrDescendantMatchesWithSpecs(child, inheritedSpecs, visited))
                            continue;
                    }
    ...
    std::vector<std::pair<int64_t, std::string>> getAllItems()
    {
        ...
        if (dbBackend && dbBackend->isOpen())
        {
            ...
            if (!stmt)
            {
                PLOGW << "getAllItems prepare failed: " << err;
                return out;
            }
            auto rs = stmt->executeQuery();
            while (rs && rs->next())
    ...
    std::vector<int64_t> getParentsOf(int64_t childID)
    {
        ...
        if (dbBackend && dbBackend->isOpen())
        {
            ...
            if (!stmt)
            {
                PLOGW << "getParentsOf prepare failed: " << err;
            }
            else
            {
    ...
    bool addParentRelation(int64_t parentID, int64_t childID)
    {
        ...
        if (dbBackend && dbBackend->isOpen())
        {
            ...
            if (!chk)
            {
                PLOGW << "addParentRelation: prepare check failed: " << err;
                return false;
            }
            ...
            if (!ins)
            {
                PLOGW << "addParentRelation: prepare insert failed: " << err;
                return false;
            }
            ...
            if (!ins->execute())
            {
                PLOGW << "addParentRelation: insert execute failed";
                return false;
            }
            return true;
        }
    ...
    bool removeParentRelation(int64_t parentID, int64_t childID)
    {
        ...
        if (dbBackend && dbBackend->isOpen())
        {
            ...
            if (!cnt)
            {
                PLOGW << "removeParentRelation: count prepare failed: " << err;
                return false;
            }
            ...
            if (!del)
            {
                PLOGW << "removeParentRelation: delete prepare failed: " << err;
                return false;
            }
            ...
            if (!del->execute())
            {
                PLOGW << "removeParentRelation: delete execute failed";
                return false;
            }
            return true;
        }
    ...
    {
        size_t l = 0;
        while (l < s.size() && std::isspace((unsigned char)s[l]))
            ++l;
        size_t r = s.size();
    ...
    std::vector<std::string> collectTagsFromPath(const std::vector<int64_t> &path)
    {
        ...
        for (int i = (int)path.size() - 1; i >= 0; --i)
        {
            ...
            try
            {
                ...
                {
                    std::string low = toLowerCopy(t);
                    if (seenLower.insert(low).second)
                        out.push_back(t);
                }
            }
            catch (...)
    ...
    bool setTagsFor(int64_t id, const std::vector<std::string> &tags)
    {
        ...
        if (history)
        {
            ...
            std::string rid = history->recordRevision(id, author, "update", fields);
            if (rid.empty())
                PLOGW << "recordRevision failed for setTagsFor id=" << id;
        }
        return true;
    }
    ...
    std::string getTagsOf(int64_t id)
    {
        ...
        if (sqlite3_prepare_v2(dbConnection, sql, -1, &stmt, nullptr) == SQLITE_OK)
        {
            ...
            if (sqlite3_step(stmt) == SQLITE_ROW)
            {
                const unsigned char *text = sqlite3_column_text(stmt, 0);
                if (text)
                    tags = reinterpret_cast<const char *>(text);
            }
        }
        if (stmt)
            sqlite3_finalize(stmt);
    ...
    bool renameItem(int64_t id, const std::string &newName)
    {
        ...
        if (history)
        {
            ...
            std::string rid = history->recordRevision(id, author, "update", fields);
            if (rid.empty())
                PLOGW << "recordRevision failed for renameItem id=" << id;
        }
        return true;
    }
    ...
    bool deleteItem(int64_t id)
    {
        ...
        if (history)
        {
            ...
            if (rid.empty())
                PLOGW << "recordRevision failed for delete id=" << id;
        }
    ...
    std::string sanitizeContent(const std::string &str)
    {
        // Robust sanitizer:
        // - Decode UTF-8 and skip invalid sequences
        // - Map common smart punctuation to ASCII equivalents
        // - Remove invisible/zero-width chars and most control characters
        // - Normalize CR or CRLF to \n and convert NBSP to space
        ...
        while (i < str.size())
        {
            ...
            // Ensure full sequence present
            if (i + need > str.size())
            {
                break;
            } // truncated at end: drop remainder
            bool valid = true;
            ...
            // Detect overlongs and illegal ranges
            if ((need == 2 && cp < 0x80) || (need == 3 && cp < 0x800) || (need == 4 && cp < 0x10000) || (cp > 0x10FFFF) || (cp >= 0xD800 && cp <= 0xDFFF))
            {
                i += need;
                continue; // invalid encoding or surrogate, skip
            }

            // Map / filter codepoints
            // Remove ZERO WIDTH / Invisibles, BOM, etc.
            bool appended = false;
            ...
            case 0x2026: // …
                ...
                appended = true;
                break;
            case 0xFFFD: // replacement char
                // drop
                appended = true;
                break;
            case 0x00A0: // NBSP -> space
                out.push_back(' ');
                appended = true;
                break;
            case 0xFEFF: // BOM - drop
                appended = true;
                break;
            case 0x200B:
            case 0x200C:
            case 0x200D:
            case 0x2060: // zero-width / word joiner
                appended = true;
                break;
            case 0x000D: // CR -> normalize to LF and also skip immediate LF if present
                out.push_back('\n');
                appended = true;
                if (i + need < str.size() && str[i + need] == '\n')
                {
                    i += need + 1;
                    continue; // skip LF of CRLF
                }
                break;
            default:
                if (cp < 0x20 && cp != '\n' && cp != '\t')
        ...
        // Collapse repeated spaces and trim trailing spaces on lines (optional): here we condense sequences of \r/\n already normalized
        // Replace any remaining CR with LF (defensive)
        for (size_t p = 0; p < out.size(); ++p)
            if (out[p] == '\r')
                out[p] = '\n';

        // Replace double spaces with single (preserve intent but keep things tidy)
        std::string finalOut;
...
inline int64_t Vault::addAttachment(int64_t itemID, const std::string &name, const std::string &mimeType, const std::vector<uint8_t> &data, const std::string &externalPath)
{
    ...
    if (dbBackend && dbBackend->isOpen())
    {
        ...
        if (!stmt)
        {
            PLOGE << "addAttachment prepare failed: " << err;
            return -1;
        }
        if (itemID >= 0)
        ...
        if (!data.empty())
            stmt->bindBlob(4, reinterpret_cast<const void *>(data.data()), data.size());
        else
            stmt->bindNull(4);
        if (!externalPath.empty())
            stmt->bindString(5, externalPath);
        else
            stmt->bindNull(5);
        stmt->bindInt(6, static_cast<int64_t>(data.size()));
        if (!stmt->execute())
        {
            PLOGE << "addAttachment execute failed";
            return -1;
        }
        return dbBackend->lastInsertId();
    }
    ...
    if (sqlite3_prepare_v2(dbConnection, sql, -1, &stmt, nullptr) == SQLITE_OK)
    {
        ...
        if (!data.empty())
            sqlite3_bind_blob(stmt, 4, reinterpret_cast<const void *>(data.data()), static_cast<int>(data.size()), SQLITE_TRANSIENT);
        else
            sqlite3_bind_null(stmt, 4);
        if (!externalPath.empty())
            sqlite3_bind_text(stmt, 5, externalPath.c_str(), -1, SQLITE_TRANSIENT);
        else
...
inline Vault::Attachment Vault::getAttachmentMeta(int64_t attachmentID)
{
    ...
    if (dbBackend && dbBackend->isOpen())
    {
        ...
        if (!stmt)
        {
            PLOGE << "getAttachmentMeta prepare failed: " << err;
            return a;
        }
    ...
    if (sqlite3_prepare_v2(dbConnection, sql, -1, &stmt, nullptr) == SQLITE_OK)
    {
        ...
        if (step == SQLITE_ROW)
        {
            ...
            if (sqlite3_column_type(stmt, 7) != SQLITE_NULL)
                a.displayWidth = static_cast<int>(sqlite3_column_int(stmt, 7));
            if (sqlite3_column_type(stmt, 8) != SQLITE_NULL)
                a.displayHeight = static_cast<int>(sqlite3_column_int(stmt, 8));
            PLOGV << "vault:getAttachmentMeta got row id=" << a.id << " name='" << a.name << "' size=" << a.size << " externalPath='" << a.externalPath << "' mime='" << a.mimeType << "'";
        }
        else
        {
...
inline std::vector<Vault::Attachment> Vault::listAttachments(int64_t itemID)
{
    ...
    if (dbBackend && dbBackend->isOpen())
    {
        ...
        if (!stmt)
        {
            PLOGE << "listAttachments prepare failed: " << err;
            return out;
        }
        stmt->bindInt(1, itemID);
        auto rs = stmt->executeQuery();
...
inline std::vector<uint8_t> Vault::getAttachmentData(int64_t attachmentID)
{
    ...
    if (dbBackend && dbBackend->isOpen())
    {
        ...
        if (!stmt)
        {
            PLOGE << "getAttachmentData prepare failed: " << err;
            return out;
        }
        stmt->bindInt(1, attachmentID);
        auto rs = stmt->executeQuery();
    ...
    if (sqlite3_prepare_v2(dbConnection, sql, -1, &stmt, nullptr) == SQLITE_OK)
    {
        ...
        if (sqlite3_step(stmt) == SQLITE_ROW)
        {
            ...
            if (blob && bsize > 0)
            {
                out.resize(static_cast<size_t>(bsize));
                memcpy(out.data(), blob, static_cast<size_t>(bsize));
            }
        }
    }
    if (stmt)
...
inline bool Vault::removeAttachment(int64_t attachmentID)
{
    if (dbBackend && dbBackend->isOpen())
    {
        ...
        if (!stmt)
        {
            PLOGE << "removeAttachment prepare failed: " << err;
            return false;
        }
        stmt->bindInt(1, attachmentID);
        return stmt->execute();
...
inline bool Vault::setAttachmentDisplaySize(int64_t attachmentID, int width, int height)
{
    if (dbBackend && dbBackend->isOpen())
    {
        ...
        if (!stmt)
        {
            PLOGE << "setAttachmentDisplaySize prepare failed: " << err;
            return false;
        }
        if (width >= 0)
            stmt->bindInt(1, width);
        else
            stmt->bindNull(1);
        if (height >= 0)
        ...
        if (ok)
        {
            statusMessage = "Display settings saved";
            statusTime = ImGui::GetTime();
        }
        return ok;
    }
...
inline void Vault::openPreviewFromSrc(const std::string &src)
{
    ...
    if (src.rfind(prefix, 0) == 0)
    {
        ...
        try
        {
            ...
            statusMessage = "Attachment empty";
            statusTime = ImGui::GetTime();
        }
        catch (...)
        {
    ...
    if (src.rfind("http://", 0) == 0 || src.rfind("https://", 0) == 0)
    {
        ...
        if (aid != -1)
        {
            ...
            {
                statusMessage = "Fetching web resource...";
                statusTime = ImGui::GetTime();
        ...
        {
            statusMessage = "Failed to create cache record";
            statusTime = ImGui::GetTime();
            return;
        }
    }
    try
    {
        if (std::filesystem::exists(path) && std::filesystem::is_regular_file(path))
        {
            ...
            if (in)
            {
                ...
                std::transform(ext.begin(), ext.end(), ext.begin(), ::tolower);
                if (ext == ".png" || ext == ".jpg" || ext == ".jpeg" || ext == ".bmp" || ext == ".gif")
                    previewMime = std::string("image/") + (ext.size() > 1 ? ext.substr(1) : "");
                else
                    previewMime = "application/octet-stream";
                previewIsRaw = true;
                showAttachmentPreview = true;
                previewAttachmentID = -1;
                ImGui::OpenPopup("Attachment Preview");
                return;
            }
        }
    ...
    statusMessage = "Unsupported preview source";
    statusTime = ImGui::GetTime();
}
...
inline void Vault::openModelFromSrc(const std::string &src)
{
    ...
    if (src.rfind("http://", 0) == 0 || src.rfind("https://", 0) == 0)
    {
        ...
        statusMessage = "Failed to prepare remote model";
        statusTime = ImGui::GetTime();
        return;
    }
    ...
    statusMessage = "Unsupported model source";
    statusTime = ImGui::GetTime();
}
        ...
        if (pos != std::string::npos)
        {
            ...
            if (sqlite3_prepare_v2(dbConnection, ssql, -1, &sstmt, nullptr) == SQLITE_OK)
            {
                ...
                if (sqlite3_step(sstmt) == SQLITE_ROW)
                    out = sqlite3_column_int64(sstmt, 0);
            }
            if (sstmt)
        ...
        if (!sbase.empty())
        {
            ...
            if (sqlite3_prepare_v2(dbConnection, likeSQL, -1, &lstmt, nullptr) == SQLITE_OK)
            {
                ...
                if (sqlite3_step(lstmt) == SQLITE_ROW)
                    out = sqlite3_column_int64(lstmt, 0);
            }
            if (lstmt)
...
inline ModelViewer *Vault::getOrCreateModelViewerForSrc(const std::string &src)
{
    ...
    if (src.rfind(prefix, 0) == 0)
    {
        try
        {
            int64_t aid = std::stoll(src.substr(prefix.size()));
            auto meta = getAttachmentMeta(aid);
            if (meta.size > 0)
            {
                statusMessage = "Loading model...";
    ...
    ModelViewer *out = mv.get();
    modelViewerCache[src] = std::move(mv);
    return out;
}
...
inline bool Vault::fetchAttachmentNow(int64_t attachmentID, const std::string &url)
{
    ...
    if (mime.empty())
    {
        ...
        try
        {
            ...
            std::transform(ext.begin(), ext.end(), ext.begin(), ::tolower);
            if (!ext.empty())
                mime = (ext == ".png" || ext == ".jpg" || ext == ".jpeg" ? std::string("image/") + (ext.size() > 1 ? ext.substr(1) : "") : std::string("application/octet-stream"));
        }
        catch (...)
        {
        }
    ...
    // Update DB with blob and mime and size
    {
        ...
        {
            if (!out.empty())
                sqlite3_bind_blob(stmt, 1, reinterpret_cast<const void *>(out.data()), static_cast<int>(out.size()), SQLITE_TRANSIENT);
            else
                sqlite3_bind_null(stmt, 1);
...
inline int64_t Vault::addAssetFromFile(const std::string &filepath, const std::string &desiredName)
{
    ...
    if (existing != -1)
    {
        ...
        statusMessage = "Asset exists: confirm action";
        statusTime = ImGui::GetTime();
        return -1;
    }
...
inline bool Vault::hasUsers() const
{
    if (dbBackend && dbBackend->isOpen())
    {
        ...
        if (!stmt)
        {
            PLOGE << "hasUsers prepare failed: " << err;
            return false;
        }
        auto rs = stmt->executeQuery();
        return rs && rs->next();
...
inline int64_t Vault::createUser(const std::string &username, const std::string &displayName, const std::string &passwordPlain, bool isAdmin)
{
    ...
    if (dbBackend && dbBackend->isOpen())
    {
        ...
        if (!stmt)
        {
            PLOGE << "createUser prepare failed: " << err;
            return -1;
        }
...
inline int64_t Vault::authenticateUser(const std::string &username, const std::string &passwordPlain)
{
    ...
    while (!uname.empty() && std::isspace((unsigned char)uname.front()))
        uname.erase(uname.begin());
    while (!uname.empty() && std::isspace((unsigned char)uname.back()))
        uname.pop_back();
    ...
    if (dbBackend && dbBackend->isOpen())
    {
        ...
        if (!stmt)
        {
            PLOGE << "authenticateUser prepare failed: " << err;
            return -1;
        }
...
inline void Vault::setCurrentUser(int64_t userID)
{
    ...
    if (dbBackend && dbBackend->isOpen())
    {
        ...
        if (!stmt)
        {
            PLOGE << "setCurrentUser prepare failed: " << err;
            return;
        }
...
inline void Vault::clearCurrentUser()
{
    currentUserID = -1;
    currentUserDisplayName.clear();
}
...
inline std::vector<Vault::User> Vault::listUsers() const
{
    ...
    if (dbBackend && dbBackend->isOpen())
    {
        ...
        if (!stmt)
        {
            PLOGE << "listUsers prepare failed: " << err;
            return out;
        }
        auto rs = stmt->executeQuery();
        while (rs && rs->next())
...
inline bool Vault::isUserAdmin(int64_t userID) const
{
    ...
    if (dbBackend && dbBackend->isOpen())
    {
        ...
        if (!stmt)
        {
            PLOGE << "isUserAdmin prepare failed: " << err;
            return false;
        }
        stmt->bindInt(1, userID);
        auto rs = stmt->executeQuery();
...
inline bool Vault::deleteUser(int64_t userID)
{
    ...
    if (sqlite3_prepare_v2(dbConnection, adminCheck, -1, &stmt, nullptr) == SQLITE_OK)
    {
        sqlite3_bind_int64(stmt, 1, userID);
        if (sqlite3_step(stmt) == SQLITE_ROW)
            targetIsAdmin = sqlite3_column_int(stmt, 0) != 0;
    }
    if (stmt)
        sqlite3_finalize(stmt);
    if (targetIsAdmin)
    {
        ...
        if (sqlite3_prepare_v2(dbConnection, countSql, -1, &cstmt, nullptr) == SQLITE_OK)
        {
            if (sqlite3_step(cstmt) == SQLITE_ROW)
                cnt = sqlite3_column_int(cstmt, 0);
        }
        if (cstmt)
...
inline bool Vault::setItemPermission(int64_t itemID, int64_t userID, int level)
{
    ...
    if (sqlite3_prepare_v2(dbConnection, aSql, -1, &astmt, nullptr) == SQLITE_OK)
    {
        ...
        if (sqlite3_step(astmt) == SQLITE_ROW)
            targetIsAdmin = (sqlite3_column_int(astmt, 0) != 0);
    }
    if (astmt)
        sqlite3_finalize(astmt);
    if (targetIsAdmin && level < 2)
        level = 2; // force EDIT for admins
...
inline std::vector<std::pair<int64_t, std::string>> Vault::getAllItemsForUser(int64_t userID)
{
    ...
    for (auto &p : all)
    {
        if (isItemVisibleToUser(p.first, userID))
            out.push_back(p);
    }
    return out;
}
