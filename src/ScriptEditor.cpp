...
bool ConsumePendingOpenScriptEditor(std::string *outExternalPath)
{
	if (s_pendingOpenScriptPath.empty())
		return false;
	if (outExternalPath)
		*outExternalPath = s_pendingOpenScriptPath;
	s_pendingOpenScriptPath.clear();
	return true;
}

bool HasPendingOpenScriptEditor() { return !s_pendingOpenScriptPath.empty(); }

void RenderScriptEditor(Vault *vault, bool *pOpen)
{
	// If there's a pending request, ensure the window is opened
	if (!s_pendingOpenScriptPath.empty() && pOpen)
		*pOpen = true;

	// Ensure Script Editor opens on the main viewport and docks into the main
	// dockspace
	if (ImGui::GetMainViewport())
		ImGui::SetNextWindowViewport(ImGui::GetMainViewport()->ID);
	ImGui::SetNextWindowDockID(ImGui::GetID("MyDockSpace"),
							   ImGuiCond_FirstUseEver);
	if (!ImGui::Begin("Script Editor", pOpen))
	{
		ImGui::End();
		return;
	}
	...
	{
		ImGui::TextDisabled("Open a vault to edit scripts.");
		ImGui::End();
		return;
	}
	...
	if (!s_pendingOpenScriptPath.empty())
	{
		...
		{
			// Pass the vault URI directly to LuaEditor; FileUri parsing will handle it and route through the VaultFileBackend
			Lua::LuaEditor::get().openFile(std::filesystem::path(req));
		}
	}

	// Draw the Lua editor UI (it will manage its own tabs/state)
	// get width/height available
	ImVec2 avail = ImGui::GetContentRegionAvail();
	// create a child that takes 80% available space for the editor
	ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0, 0));
	ImGui::BeginChild("LuaEditorChild", {avail.x, avail.y* 0.6f}, true,
					  ImGuiWindowFlags_NoScrollWithMouse |
						  ImGuiWindowFlags_NoScrollbar);
	Lua::LuaEditor::get().draw();
	ImGui::EndChild();
	ImGui::PopStyleVar();
	Lua::LuaEditor::get().drawConsole();
